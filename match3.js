...
(function() {
    ...
    // ДОБАВЛЕНО: ПРЕДЗАГРУЗКА ГРАФИКИ ФИШЕК И ПРЕПЯТСТВИЙ
    // Раньше картинки подгружались браузером "по требованию" — то есть именно в тот момент,
    // когда конкретный тип фишки первый раз появлялся на поле. На мобильном интернете это
    // ощущалось как "тяжёлые, долго грузящиеся картинки" (пустая/белая клетка на пару кадров,
    // пока грузится PNG). Теперь мы запускаем загрузку ВСЕХ картинок фишек одним пакетом сразу
    // при открытии игры — браузер закачивает и кэширует их параллельно в фоне, и к моменту,
    // когда игрок доходит до первого уровня, картинки уже лежат в кэше и вставляются мгновенно.
    (function preloadTileArtwork() {
        const files = [
            'heart','bullet','garlic','stake','vial','coin','donut',
            'box_1','box_2','box_3',
            'vase_1','vase_2',
            'nut_1','nut_2','nut_3',
            'cookie_1','cookie_2','cookie_3',
            'plaid_1','plaid_2','plaid_3','plaid_4',
            'ringCase_1','ringCase_2','ringCase_3',
            'stone_1','stone_2','stone_3',
            'surpriseBox_1','surpriseBox_2','surpriseBox_3','surpriseBox_4','surpriseBox_5',
            'carpetRoll_6','ribbon','ivy','foam','purpleFoam',
            'ice_1','ice_2','chain_1','chain_2',
            'bomb','plane','rainbow','rocketRow','rocketCol',
            'carpet'
        ];
        files.forEach(name => {
            const img = new Image();
...
// Движок пошаговой гравитации (ИСПРАВЛЕННЫЙ БАЛАНС СКОБОК)
    function applyGravityAndRefill(){
        ...
        // 3. ИСПРАВЛЕНО: "СТРАХОВОЧНЫЙ" ПРОХОД — закрывает дыры, которые не может достать
        // ни спуск сверху, ни диагональное сползание (шаг 1), ни спавн по верху столбца (шаг 2).
        // Такое случается на фигурных картах: пустая ячейка оказывается "в кармане", со всех
        // сторон отгороженная неподвижными препятствиями (ящики/лёд/цепи), и в неё физически
        // неоткуда упасть фишке. Раньше это навсегда оставляло в поле дыру ("поле дырявится").
        // Теперь любая оставшаяся пустая, но игровая (не 0 в карте) клетка досоздаётся на месте.
        for (let r = 0; r < SIZE; r++) {
            for (let c = 0; c < SIZE; c++) {
                if (levelLayout[r][c] !== 0 && grid[r][c] === null) {
                    let spawnType = weightedRandType();
                    grid[r][c] = createTile(r, c, spawnType, Math.max(0, r - 1));
                    moved = true;
                }
            }
        }
